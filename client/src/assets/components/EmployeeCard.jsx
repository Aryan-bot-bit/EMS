...
const EmployeeCard = ({employee, onDelete, onEdit}) => { // eslint-disable-line no-unused-vars
  ...
  return (
    ...
    {employee.isDeleted && <span className='bg-red-500/60 font-medium text-white px-2.5 py-1 text-xs rounded'>
        DELETED
    </span>}
    </div>
...
<button onClick={handleDelete} className='p-2.5 bg-white/90 backdrop-blur-sm text-slate-700 hover:text-rose-600 rounded-xl shadow-lg transition-all hover:scale-105 disabled:opacity-50'>
    <Trash2Icon className="w-4 h-4"/>
      ...
      <div className='p-5'>
        ...
        <p className='text-xs text-slate-500'>{employee.position}
